...
class NHolidayJp {
  ...
  static String getName(int year, int month, int date) {
    ...
    final holiday = holidays.firstWhereOrNull(
  ...
  static List<Holiday> create(int year) {
    ...
    (constants['fixed'] as List).forEach((v) {
      if (v['from'] <= year && (v['to'] == null || year <= v['to'])) {
        holidays
            .add(Holiday(month: v['month'], date: v['date'], name: v['name']));
      }
    });
    ...
    (constants['happyMonday'] as List).forEach((v) {
      if (v['from'] <= year && (v['to'] == null || year <= v['to'])) {
        holidays.add(Holiday(
            month: v['month'],
            date: NHolidayJp.getDateByTurn(year, v['month'], v['turn']),
            name: v['name']));
      }
    });
    ...
    // 国民の祝日(春分の日、秋分の日)
    holidays.add(Holiday(
        month: 3, date: NHolidayJp.getEquinox(year, 'shunbun'), name: '春分の日'));
    holidays.add(Holiday(
        month: 9, date: NHolidayJp.getEquinox(year, 'shuubun'), name: '秋分の日'));
    ...
    // 国民の休日
    final kokuminNoKyuujitsuList =
        holidays.fold(<Holiday>[], (List<Holiday> a, v) {
      final kokuminNoKyuujitsu =
          NHolidayJp.getKokuminNoKyuujitsu(holidays, v, year, furikaeList);
  ...
  /// [holiday] 国民の祝日
  /// [year] 年
  static Holiday? getFurikae(
      List<Holiday> holidays, Holiday holiday, int year) {
    var date = DateTime(year, holiday.month, holiday.date);
    if (date.weekday != DateTime.sunday ||
        date.isBefore((constants['furikaeKyuujitsu'] as Map)['from'])) {
      return null;
    }
    while (holidays.firstWhereOrNull(
                (v) => (v.month == date.month && v.date == date.day)) !=
            null ||
    ...
    return Holiday(
        month: date.month,
        date: date.day,
        name: (constants['furikaeKyuujitsu'] as Map)['name']);
  }
  ...
  static Holiday? getKokuminNoKyuujitsu(
    List<Holiday> holidays,
    Holiday holiday,
    int year,
    List<Holiday> furikaeList,
  ) {
    ...
    if (holidays.firstWhereOrNull(
              (v) => (v.month == nextDate.month && v.date == nextDate.day),
            ) ==
            null &&
        furikaeList.firstWhereOrNull(
              (v) => (v.month == nextDate.month && v.date == nextDate.day),
            ) ==
            null &&
        nextDate.weekday != DateTime.sunday &&
        holidays.firstWhereOrNull(
              (v) => (v.month == next2Date.month && v.date == next2Date.day),
            ) !=
            null) {
      return Holiday(
          month: nextDate.month,
          date: nextDate.day,
          name: (constants['kokuminNoKyuujitsu'] as Map)['name']);
    }
    return null;
  }
  ...
  static int getDateByTurn(int year, int month, int turn) {
    return (8 - (DateTime(year, month, 1)).weekday % 7) % 7 +
        1 +
        (turn - 1) * 7;
  }
  ...
  static int getEquinox(int year, String type) {
    return ((constants[type] as List).firstWhere(
            (v) => v['from'] <= year && (v['to'] == null || year <= v['to']))
        as Map)['mod4'][year % 4];
  }
}
...
class Holiday {
  ...
  /// [date] 日 1から31
  /// [name] 名称
  Holiday({required this.month, required this.date, required this.name});
